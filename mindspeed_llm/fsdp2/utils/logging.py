...
class _Rank0LoggerAdapter(logging.LoggerAdapter):
    """Add MindSpeed rank-aware helpers to third-party logger classes.

    Some environments install an ``EventLogger`` as Python's global logger
    class before MindSpeed is imported. Existing logger instances then do not
    gain the methods defined by ``_Logger`` even after ``setLoggerClass``.
    """
    ...
    @lru_cache(None)
    def warn_once(self, msg: str, *args, **kwargs) -> None:
        self.warn_rank0(msg, *args, **kwargs)
    ...
    def info_plain_rank0(self, msg: str, *args, **kwargs) -> None:
        if int(os.getenv("LOCAL_RANK", "0")) == 0:
            self.info_plain(msg, *args, **kwargs)
...
def get_logger(name: Optional[str] = None, level: Optional[str] = "INFO") -> _Logger:
    ...
    if not hasattr(logger, "info_rank0"):
        return _Rank0LoggerAdapter(logger, {})
    return logger
