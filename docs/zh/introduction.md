...
MindSpeed LLM支持Transformer架构的大型语言模型（LLM，Large Language Model），并支持MoE模型的训练和调优。提供超过100个主流开源模型，以及开箱即用的模型训练脚本。
...
- **MindSpeed LLM模型支持**  
    - **预制100+开源模型**：涵盖Dense/MoE/SSM主流系列模型，提供高性能模型训练脚本，开箱即用。
