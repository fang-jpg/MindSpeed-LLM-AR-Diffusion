...
检查第 1 层（层索引0）实际传给 attention kernel 的完整 mask。发现 0 个可见关系错误。允许的边和禁止的边均符合当前 block_diff 规则。 Q形状=[1, 4, 24, 16]，分别为batch、query头数、位置数、每头维度；K形状=[1, 2, 24, 16]，第二维为key头数。mask=[1, 1, 24, 24]，最后两维分别是query行和key列。 is_causal=false表示不额外套普通causal三角限制，实际可见性仍由4D mask控制；dropout=0表示关闭随机丢弃。 第一层示例query=4，可见key=[4, 5, 6, 7, 12, 13, 14, 15]；位置映射为noisy 0–11，clean 12–23。
...
检查第 2 层（层索引1）实际传给 attention kernel 的完整 mask。发现 0 个可见关系错误。允许的边和禁止的边均符合当前 block_diff 规则。 Q形状=[1, 4, 24, 16]，分别为batch、query头数、位置数、每头维度；K形状=[1, 2, 24, 16]，第二维为key头数。mask=[1, 1, 24, 24]，最后两维分别是query行和key列。 is_causal=false表示不额外套普通causal三角限制，实际可见性仍由4D mask控制；dropout=0表示关闭随机丢弃。
...
只替换clean Block0（拼接位置12–15），固定全部noisy，观察noisy Block1（位置4–7）全部输出。 最大绝对差max_abs=0.47576940059661865；平均绝对差mean_abs=0.12542593479156494；整体L2差异l2=3.604879856109619；阈值=1e-06。这是允许的过去clean前缀，max_abs大于阈值说明正对照有效、前缀确实影响输出；非零是期望结果。
...
```json
{
  ...
  "status": "PASS",
  "max_abs": 0.47576940059661865,
  "mean_abs": 0.12542593479156494,
  "l2": 3.604879856109619,
  "threshold": 1e-06,
  "expected": "应变化（正对照）"
}
```
...
只替换clean Block1（拼接位置16–19），固定全部noisy，观察noisy Block1（位置4–7）全部输出。 最大绝对差max_abs=0.0；平均绝对差mean_abs=0.0；整体L2差异l2=0.0；阈值=1e-06。这是应禁止的依赖；实测差异为0，符合应不变的要求。
...
| 4–7 | noisy Block1 | 允许 | 3.164376974 | 3.971794605 |
| 8–11 | noisy Block2 | 禁止 | 0 | 0 |
| 12–15 | clean Block0 | 允许 | 1.852002501 | 2.933556557 |
| 16–19 | clean Block1 | 禁止 | 0 | 0 |
| 20–23 | clean Block2 | 禁止 | 0 | 0 |
...
这是允许的clean前缀正对照，前缀输入位置的最大embedding梯度范数为2.93355655670166，大于阈值1e-08。说明输出存在可检测的前缀依赖，非零是预期行为。取最大值覆盖所有batch；不要求每个前缀位置都非零。
...
|---|---|---|---:|---:|
| 0–3 | noisy Block0 | 禁止 | 0 | 0 |
| 4–7 | noisy Block1 | 允许 | 2.876446009 | 4.633569717 |
| 8–11 | noisy Block2 | 禁止 | 0 | 0 |
| 12–15 | clean Block0 | 允许 | 2.254761457 | 3.815617561 |
| 16–19 | clean Block1 | 禁止 | 0 | 0 |
| 20–23 | clean Block2 | 禁止 | 0 | 0 |
...
这是允许的clean前缀正对照，前缀输入位置的最大embedding梯度范数为3.815617561340332，大于阈值1e-08。说明输出存在可检测的前缀依赖，非零是预期行为。取最大值覆盖所有batch；不要求每个前缀位置都非零。
